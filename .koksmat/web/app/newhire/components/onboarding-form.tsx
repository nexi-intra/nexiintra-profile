...
export function ValidateNewHireEmailAccountForm() {
  ...
  async function onSubmit(data: ProfileFormValues) {
    ...
    // })
    setInvitationStatus(undefined);
    const x = await createInvitation(data, true);
    ...
    location.href =
      "https://christianiabpos.sharepoint.com/sites/welcome-to-nexi";
  ...
  return (
    <div>
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormDescription className="max-w-screen-sm">
                  Once you have entered your email address, you will be
                  redirected to a login page. If this is the first time, you
                  will have a few additional steps to complete.
                </FormDescription>
                {invitationStatus && invitationStatus.hasError && (
                  <div className="rounded-lg bg-red-600 p-5 font-bold text-white">
      ...
      {isNewGuest && (
        <NewGuest
          continueToSignin={async function () {
            const signedIn = await magicbox.signIn(
              ["User.Read"],
              form.getValues("email") as string
            );
